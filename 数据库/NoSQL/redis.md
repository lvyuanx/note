...
![image-20210725145118793](redis.assets/image-20210725145118793.png) 
...
![image-20210727123506266](redis.assets/image-20210727123506266.png) 
...
![image-20210727140622787](redis.assets/image-20210727140622787.png) 
...
![image-20210727141453920](redis.assets/image-20210727141453920.png) 
...
![image-20210727144423871](redis.assets/image-20210727144423871.png) 

**如果之前的主机回来了，哨兵会自动将之前的主机变成现在主机的从机**

![image-20210727144645784](redis.assets/image-20210727144645784.png) 
...
![image-20210727145551775](redis.assets/image-20210727145551775.png) 
...
* 白名单策略
  ...
  * 提前预热各种分类数据据id对应的bitmaps，id作为bitmaps的offset，相当于设置了数据白名单。当加载正常数据时，放行，加
...
* 实时监控
  ...
  * 实时监控redis命中率（业务正常范围时，通常会有一个波动值）与null 数据的占比
    ...
    * 注意：根据倍数不同，启动不同的排查流程，然后使用黑名单进行防控（运维）
...
![image-20210727150647473](redis.assets/image-20210727150647473.png) 
...
这个思想的含义是,既然redis有可能挂掉,那我多增设几台 redis,这样一合掉之后其他的还可以继续工作,其实就是搭建的集群
...
1. setnx和expire分2步执行，非原子操作；若setnx执行成功，但expire执行失败，就可能出现死锁
   ...
   > 解决方案：
   >
   > 使用命令 set key value nx ex second，可实现原子性操作。
   >
   > nx 表示设置锁
   >
   > ex 表示设置时间
